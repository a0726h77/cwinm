...
require_once("ap_node_list.class.php");
...
if(count($argv) != 2)
{
	echo "Example Usage: {$argv[0]} ath0\n";
	exit(1);
}
else if(($uid = posix_getuid()) != 0)
{
	echo "Operation not permitted : UID=$uid\n";
	exit(2);
}

error_reporting(E_ALL);

$ap_node_list = new ap_node_list($argv[1]);
$ap_node_list->scan();

$ap_node_list->get_format_node();
...
$result = menu_select(
	array (
		'items' => $ap_node_list->get_format_node(),
		'rows' => $rows,
		'cols' => $cols,
		'selected' => 3,
		'centered' => 1
...
if($result)
{
	...
	echo $option;
	if(empty($option) || strtolower($option) == 'y')
	{
		$cmd = "iwconfig {$argv[1]} $result; dhclient $argv[1]\n";
		system($cmd);
	}
}
